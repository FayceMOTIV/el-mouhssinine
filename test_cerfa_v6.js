// Test script to generate CERFA v6 PDF (1 page + cachet simulé)
const path = require('path');
const PDFDocument = require(path.join(__dirname, 'functions/node_modules/pdfkit'));
const fs = require('fs');

const outputPath = path.join(__dirname, 'test_cerfa_v6.pdf');
...
doc.text('El Mouhssinine', cachetX + 10, doc.y + 2, { width: cachetSize - 20, align: 'center' });
doc.fontSize(5).font('Helvetica');
doc.text('Bourg-en-Bresse', cachetX + 10, doc.y + 2, { width: cachetSize - 20, align: 'center' });
doc.text('W012004130', cachetX + 10, doc.y, { width: cachetSize - 20, align: 'center' });
doc.restore();
doc.fillColor('#000000');

console.log('Y after cachet:', doc.y);
...
stream.on('finish', () => {
  ...
  console.log('  ✅ Article 200 du CGI (sans 978 dans le titre)');
  console.log('  ✅ Phrase absence contrepartie');
  console.log('  ✅ Cachet simulé rond');
  console.log('  ✅ Footer sur page 1');
  console.log('Open with: open test_cerfa_v6.pdf');
});
