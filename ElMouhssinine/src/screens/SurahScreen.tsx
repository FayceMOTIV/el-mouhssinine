...
const SurahScreen: React.FC<SurahScreenProps> = ({ route, navigation }) => {
  ...
  const handleCopyAyah = (ayahNumber: number) => {
    ...
    if (ayah) {
      ...
      }\n\n— ${t('surah')} ${surahInfo?.englishName}, ${t(
        'verse',
      )} ${ayahNumber}`;
  ...
  const isFavorite = (ayahNumber: number) =>
    favorites.has(`${surahNumber}:${ayahNumber}`);
  ...
  const renderAyah = useCallback(
    ({ item: ayah, index }: { item: any; index: number }) => {
        ...
        <View>
          ...
          {isSelected && (
            <View style={[styles.ayahActions, isRTL && styles.ayahActionsRTL]}>
              <TouchableOpacity
                style={[
                  styles.ayahActionButton,
                  isActive && styles.ayahActionButtonActive,
                ]}
              ...
              <TouchableOpacity
                style={[
                  styles.ayahActionButton,
                  isFavorite(ayah.numberInSurah) &&
                    styles.ayahActionButtonActive,
                ]}
                onPress={() => handleToggleFavorite(ayah.numberInSurah)}
              >
                <Text style={styles.ayahActionIcon}>
                  {isFavorite(ayah.numberInSurah) ? '❤️' : '🤍'}
                </Text>
                <Text style={styles.ayahActionText}>
                  {isFavorite(ayah.numberInSurah)
      ...
      <>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerContent}>
            <Text style={[styles.surahNumber, isRTL && styles.rtlText]}>
              {t('surah')} {surahNumber}
            </Text>
            <Text style={styles.surahArabicName}>{surahInfo?.name}</Text>
            <Text style={[styles.surahEnglishName, isRTL && styles.rtlText]}>
        ...
        <View style={[styles.optionsBar, isRTL && styles.optionsBarRTL]}>
          <TouchableOpacity
            style={[
              styles.optionButton,
              showTranslation && styles.optionButtonActive,
          ...
          >
            ...
            <Text
              style={[
                styles.optionText,
                showTranslation && styles.optionTextActive,
              ]}
            >
              {t('translation')}
            </Text>
        ...
        <View style={[styles.karaokePlayer, isRTL && styles.karaokePlayerRTL]}>
          <TouchableOpacity
            style={[
              ...
              (!verses.length || !player.isInitialized) && { opacity: 0.4 },
            ]}
          ...
          <View style={styles.quickOptions}>
            ...
            >
              <Text style={styles.quickOptionIcon}>
                {player.repeatMode === 'verse'
                  ? '🔂'
                  : player.repeatMode === 'surah'
                  ? '🔁'
                  : '➡️'}
              </Text>
            </TouchableOpacity>
    ...
    () => (
      <View
        style={[
          ...
          isRTL && styles.navigationContainerRTL,
        ]}
      >
        <TouchableOpacity
          style={[
  ...
  const keyExtractor = useCallback(
    (item: any) => item.numberInSurah.toString(),
    [],
  );
  ...
  const loadSurah = async () => {
    try {
      ...
      Alert.alert(t('error') as string, t('errorLoadingSurah') as string, [
